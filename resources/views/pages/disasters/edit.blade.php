                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="mt-4 mt-xl-0">
                                <form method="post" action="{{ route('disaster_update', $disaster->id) }}"
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <textarea class="form-control @error('description') is-invalid @enderror" name="description" id="description"
                                            rows="3" placeholder="Description">{{ old('description') ?? @$disaster->description }}</textarea>
                                        @error('description')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                            </span>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        @if (count(@json_decode($disaster->images) ?? []))
                                            <div class="my-2">
                                                <img src="{{ url('/') }}/storage/{{ @json_decode($disaster->images)[0] }}"
                                        ...
                                        <select name="category_id" id="category_id" class="form-control">
                                            ...
                                            @foreach ($categories as $item)
                                                <option value="{{ $item->id }}"
                                                    @if ((old('category_id') ?? @$disaster->disaster_category_id) == $item->id) selected @endif>{{ $item->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        @error('category_id')
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="status" id="status_active"
                                            ...
                                            <option value="active" @if ((old('status') ?? @$disaster->status) == 'active') selected @endif>Active
                                            </option>
                                            <option value="requested" @if ((old('status') ?? @$disaster->status) == 'requested') selected @endif>
                                                Requested</option>
                                            <option value="rejected" @if ((old('status') ?? @$disaster->status) == 'rejected') selected @endif>
                                                Rejected</option>
                                            <option value="inactive" @if ((old('status') ?? @$disaster->status) == 'inactive') selected @endif>
                                                Inactive</option>
                                        </select>
                                        @error('status')
                                            <span class="invalid-feedback" role="alert">
