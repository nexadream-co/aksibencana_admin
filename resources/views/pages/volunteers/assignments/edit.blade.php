                                ...
                                <form method="post" action="{{ route('volunteer_assignment_update', ["id" => $volunteer->id, "assignment_id" => $assignment->id]) }}" enctype="multipart/form-data">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="status" id="status_active" class="form-control @error('status') is-invalid @enderror">
                                            ...
                                            <option value="active" @if((old('status') ?? @$assignment->status) == 'active') selected @endif>Active</option>
                                            <option value="finished" @if((old('status') ?? @$assignment->status) == 'finished') selected @endif>Finished</option>
                                            <option value="canceled" @if((old('status') ?? @$assignment->status) == 'canceled') selected @endif>Canceled</option>
                                        </select>
                                        @error('status')
                                            <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
